...
| SOY_SAUCE | 1.5勺(22.5ml，汤勺) | _spec §2 |
...
- 调用 `_spec §6 焯水()`：鸡翅冷水下锅，加姜片、COOKING_WINE(半勺料酒)，煮开撇去浮沫，捞出用温水冲洗沥干。焯水是「垃圾回收」——清理血水和杂质，避免后续汤色浑浊。
...
4. **大火收汁（状态机终态）**：15min 后，鸡翅已熟透（筷子能轻松插透、无血水渗出，_spec §5），拣出葱姜蒜，转 FIRE_HIGH(大火)收汁。`while 汤汁未浓稠挂糊: 翻动收汁; 一旦浓稠挂糊: 立即关火`——收汁是**状态机终态判断**！`状态: 可乐液体→微沸浓缩→变稠→浓稠挂糊(终态)`，终态是汤汁浓稠可挂勺(_spec §5)，能均匀裹在鸡翅表面。不到终态不收火，过了终态(发焦发苦)就是 fatal error。加入 SALT(半小勺盐)最后调味（盐提甜，「延迟初始化」原则）。
