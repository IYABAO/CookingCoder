...
| GINGER | 5-6片(2-3mm厚，1元硬币厚度) | _spec §1 |

## 食材清单（Input）

| 食材 | 用量 | 切配规格 | 备注 |
| --- | --- | --- | --- |
| 土鸡/三黄鸡 | 半只(约600g) | 剁块(3cm见方，麻将牌) | _spec §1，选土鸡更香 |
| 干香菇 | 8朵(约30g) | 泡发后切十字花刀 | 泡香菇水留用(过滤后入汤) |
| 姜 | 1块 | 切片(2-3mm厚，1元硬币厚度)，5-6片 | _spec §1 |
| 葱 | 2根 | 切段(4-5cm，手指长) | _spec §1，出锅可撒葱花 |
| 料酒 | 1勺(15ml) | — | 焯水+去腥 |
| 盐 | 1小勺(5ml) | — | 出锅前放 |
...
- **焯水()**：调用 `_spec §6 焯水()`——鸡块冷水下锅，加 COOKING_WINE(1勺料酒)+姜片(3片)，煮开撇去浮沫，捞出用温水冲洗干净。焯水是「编译期优化」，去除血水和杂质，避免汤浑浊。
...
3. **慢炖（后台异步任务）**：转 FIRE_LOW(小火)，盖盖慢炖 STEW_TIME(90min)——慢炖=后台异步任务(Async Task)，主进程(灶火)持续低功耗运行，鸡肉中的氨基酸、脂肪、胶原蛋白持续溶出到汤中。终态=汤色浓白、鸡肉酥烂(_spec §5 汤煮好)。`while 未到 STEW_TIME: 保持 FIRE_LOW 微沸`，中途不要频繁揭盖——每次揭盖=线程阻塞(Thread Block)，热量逃逸导致炖制时间延长 5-10min。
...
| 状态测试 | 鸡肉筷子可轻松插透(_spec §5 肉类熟透)、汤面微沸无大泡、香菇咬开无硬芯 | 筷子插入+观察 |
