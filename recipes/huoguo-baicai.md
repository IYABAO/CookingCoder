...
| 姜 | 3片 | 切末(<2mm) | _spec §1 |
| 干辣椒 | 5个 | 切段(1cm) | 怕辣可减 |
| 五花肉 | 50g | 切片(2-3mm厚，1元硬币厚度) | _spec §1；可选，增香 |
| 花椒 | 1小勺(5ml) | 整粒 | 增麻香 |
...
3. **炒菜（主循环）**：下沥干的包菜碎片，FIRE_HIGH(大火)快速翻炒——这就像 `for` 循环遍历非结构化数组，每片包菜都要均匀沾油受热。`while 包菜未断生: 大火翻炒`，断生标准为颜色变深变软、无生涩味(_spec §5)，约 2min。全程大火，`if 包菜出水: 继续大火收干`，出水就是过度渲染。
