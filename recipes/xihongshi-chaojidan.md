...
| 西红柿 | 2个(约300g) | 切块(2cm见方，骰子大) | _spec §1；选熟透红软的 |
...
2. **炒西红柿（数据转换）**：锅留底油，调用 `_spec §6 爆香()`（小火下蒜末炒出香味约 30s），转 FIRE_HIGH(大火)，下西红柿块翻炒 1min 至出汁——西红柿从「固态块」转为「酱汁态」是一个状态机：生块→变软→出汁→浓稠，`while 汤汁未浓稠: 继续翻炒`。加入 SUGAR(1小勺糖)中和酸味、SALT(半小勺盐)打底、SOY_SAUCE(半勺生抽)提鲜。
