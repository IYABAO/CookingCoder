...
| DARK_SOY | 半勺(7.5ml) | _spec §2 |
| COOKING_WINE | 2勺(30ml) | _spec §2 |
| SALT | 半小勺(2.5ml) | _spec §2 |
| SUGAR_CANDY | 15g（冰糖） | — |
| VINEGAR | 1勺(15ml) | _spec §2 |
| BRAISE_TIME | 10min | — |
| WATER_BOWL | 1碗(250ml，标准饭碗) | _spec §2 |
...
| 食材 | 用量 | 切配规格 | 备注 |
| --- | --- | --- | --- |
| 带鱼 | 2条(约500g) | 去头尾内脏，切段(4-5cm，手指长) | _spec §1，选银鳞完整的 |
| 冰糖 | 15g | 整粒 | 炒糖色专用 |
| 姜 | 1块 | 切片(2-3mm厚，1元硬币) | _spec §1 |
| 蒜 | 4瓣 | 切片(2-3mm) | _spec §1 |
| 葱 | 2根 | 切段(4-5cm)+葱花 | _spec §1 |
| 八角 | 1颗 | 整粒 | |
| 干辣椒 | 2个 | 整只 | 可选，微辣 |
| 生抽 | 2勺(30ml) | — | _spec §2 |
| 老抽 | 半勺(7.5ml) | — | _spec §2 |
| 料酒 | 2勺(30ml) | — | _spec §2 |
| 醋 | 1勺(15ml) | — | _spec §2，去腥提鲜 |
| 盐 | 半小勺(2.5ml) | — | _spec §2 |
...
2. **炒糖色（状态机初始化）**：锅留底油，油温 OIL_30(三四成热)，下 SUGAR_CANDY(15g冰糖)，调用 `_spec §6 炒糖色()`——FIRE_LOW(小火)熬至冰糖融化，颜色经历 `浅黄→琥珀→枣红→冒细密小泡` 的状态流转。终态判定：枣红色+冒细密小泡，此时立即进入下一步。这是整道菜的**状态机初始化**，糖色决定了红烧的色泽基类。
...
5. **大火收汁（GC 压缩）**：10min 后，拣出葱姜八角，转 FIRE_HIGH(大火)收汁。`while 汤汁未浓稠挂糊: 轻翻收汁`——收汁是**垃圾回收压缩阶段**，把多余汤汁(内存碎片)压缩成浓稠酱汁(紧凑对象)。需要时淋入 STARCH_WATER(淀粉:水=1:3)勾芡至挂糊(_spec §5)。加入 SALT(半小勺盐)最后调味。
